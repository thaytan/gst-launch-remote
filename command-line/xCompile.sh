...
TOOLCHAIN_HOST_BIN="$BUILDROOT_PATH/$STAGING_DIR_RELATIVE/host/bin"
...
TARGET_USR_SHARE="$BUILDROOT_PATH/$TARGET_RELATIVE/usr/share"

export STAGING_DIR="$BUILDROOT_PATH/$STAGING_DIR_RELATIVE"
export STAGING_PREFIX="$TARGET/usr" # Needed by the pkg-config wrapper
export PATH="$TOOLCHAIN_HOST_BIN:$PATH"
export PKG_CONFIG_PATH="$TARGET_USR_LIB/pkgconfig:$TARGET_USR_SHARE/pkgconfig"
...
TOOLCHAIN_LDFLAGS="-L$TOOLCHAIN_USR_LIB -L$TOOLCHAIN_LIB -L$TARGET_USR_LIB -L$TARGET_LIB -Wl,--rpath-link=$TARGET_USR_LIB"

# debug
if [ $bDebug -eq 1 ]; then
	echo PKG_CONFIG_PATH="$PKG_CONFIG_PATH"
	echo TOOLCHAIN_HOST_BIN="$TOOLCHAIN_HOST_BIN"
